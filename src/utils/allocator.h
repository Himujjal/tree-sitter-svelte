/**
 * taken from https://github.com/dacez/zzzallocator/blob/master/zzzallocator.h
 * */
...
static inline void za_Print(za_Allocator *allocator) {
  ...
  for (size_t i = 0; i < za_BIN_LEVEL * za_BIN_LEVEL_SIZE; ++i) {
    ...
    printf("    [%zu] Ptr %p ChunkSize %lu Head %p FreeHead %p BNSize %zu "
           "FBNSize %zu\n",
