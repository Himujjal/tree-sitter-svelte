...
static int hashmap_put(za_Allocator *A, struct hashmap_s *const hashmap,
                       const char *const key, const unsigned len,
                       TagType value) HASHMAP_USED;
...
static int hashmap_iterate(const struct hashmap_s *const hashmap,
                           int (*f)(void *const context, TagType value),
...
static int hashmap_iterate_pairs(za_Allocator *A,
...
static void hashmap_destroy(za_Allocator *A,
                            struct hashmap_s *const hashmap) HASHMAP_USED;
...
static int hashmap_match_helper(const struct hashmap_element_s *const element,
                                const char *const key,
                                const unsigned len) HASHMAP_USED;
static int hashmap_hash_helper(za_Allocator *A, const struct hashmap_s *const m,
                               const char *const key, const unsigned len,
                               unsigned *const out_index) HASHMAP_USED;
static int
hashmap_rehash_iterator(za_Allocator *A, void *const new_hash,
                        struct hashmap_element_s *const e) HASHMAP_USED;
static int hashmap_rehash_helper(za_Allocator *A,
                                 struct hashmap_s *const m) HASHMAP_USED;
  ...
  for (i = 0; i < HASHMAP_MAX_CHAIN_LENGTH; i++) {
    if (m->data[curr].in_use) {
      if (hashmap_match_helper(&m->data[curr], key, len)) {
        ...
        m->data[curr].key = (char *)HASHMAP_NULL;
...
int hashmap_iterate(const struct hashmap_s *const m,
                    int (*f)(void *const, TagType), void *const context) {
  unsigned int i;
